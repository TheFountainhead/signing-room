<div>
    ...
    <form wire:submit="sendForSigning">
        ...
        <div class="card" style="margin-bottom: 24px;">
            ...
            <div class="form-group">
                ...
                <label for="document"
                       ...
                       ondrop="event.preventDefault(); this.style.borderColor='var(--ft-border)'; this.style.background='var(--ft-pink-light)'; if (event.dataTransfer.files.length) { const input = document.getElementById('document'); input.files = event.dataTransfer.files; input.dispatchEvent(new Event('change', { bubbles: true })); }">
                    @if($document)
        ...
        <div class="card" style="margin-bottom: 24px;">
            ...
            {{-- Internal signer dropdown --}}
            <div style="padding: 12px 16px; border: 1px solid var(--ft-border); border-radius: 8px; margin-bottom: 16px; background: {{ $internalSignerId ? '#EBF5FF' : 'white' }};">
                <label class="form-label" style="font-size: 0.9rem; margin-bottom: 8px;">Intern underskriver (valgfrit)</label>
                <select wire:model.live="internalSignerId" class="form-input">
                    <option value="">— Ingen intern underskriver —</option>
                    @foreach($admins as $admin)
                        <option value="{{ $admin->id }}">{{ $admin->name }} ({{ $admin->email }})</option>
                    @endforeach
                </select>
                <div style="font-size: 0.8rem; color: var(--ft-grey); margin-top: 4px;">Tilføjer automatisk en administrator som underskriver</div>
            </div>
